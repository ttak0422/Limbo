...
local view = {cursorline = true, debounce_delay = 50, side = "left", signcolumn = "yes", width = {min = 30, max = -1, padding = 1}, float = {quit_on_focus_loss = true, enable = false}, relativenumber = false, number = false, preserve_window_proportions = false, centralize_selection = false}
local renderer
do
  ...
  local show = {file = true, folder = true, folder_arrow = true, git = true, modified = false, bookmarks = false, diagnostics = false}
...
local diagnostics = {debounce_delay = 100, severity = {min = vim.diagnostic.severity.INFO, max = vim.diagnostic.severity.ERROR}, icons = {hint = "", info = "", warning = "", error = ""}, show_on_dirs = false, enable = false, show_on_open_dirs = false}
...
local actions
do
  local change_dir = {global = false, restrict_above_cwd = false, enable = false}
  local expand_all = {max_folder_discovery = 300, exclude = {}}
  local file_popup = {open_win_config = {col = 1, row = 1, relative = "cursor", border = "none", style = "minimal"}}
