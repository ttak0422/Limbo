...
Hydra.setup({foreign_keys = nil, color = "red", timeout = 10000, hint = {show_name = true, position = {"bottom"}, offset = 0, float_opts = {}}, on_enter = nil, on_exit = nil, on_key = nil, invoke_on_body = false, exit = false, debug = false})
do
  local heads
  local function _1_()
  ...
  local config = {invoke_on_body = true, hint = {type = "window", position = "middle", float_opts = float_opts}}
  local hint = ":Move                 :Swap     :Utils\n--------------------  -------   ------------------------\n   _k_       _<C-k>_         _K_       _e_: start resize mode\n _h_   _l_  _<C-h>_ _<C-l>_    _H_   _L_     _<CR>_ open popup window\n   _j_       _<C-j>_         _J_\n"
  Hydra({name = "Windows", mode = "n", body = "<C-w>", heads = heads, config = config, hint = hint})
end
local heads = {{"H", "<C-v>h:VBox<CR>"}, {"J", "<C-v>j:VBox<CR>"}, {"K", "<C-v>k:VBox<CR>"}, {"L", "<C-v>l:VBox<CR>"}, {"f", ":VBox<CR>", {mode = "v"}}, {"<Esc>", nil, {exit = true, desc = "close"}}}
local config
local function _3_()
